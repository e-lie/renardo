...
def multi_panning_timevar_beta(entry: TimeVar, num_channels=default_num_channels):
    output = 2
    pan = 0
    output_values = []
    pan_values = []
...
def multi_panning_simple_pattern(entry: Pattern, num_channels=default_num_channels):
    panning_func = None
    if num_channels==8:
        panning_func = octo_panning_beta
    elif num_channels==6:
...
@player_method
def mpan(self, entry, num_channels=default_num_channels):
...
def surround_panning(position=0, distance=1, num_channels=default_num_channels):
    if num_channels == 8:
        init_offset = 5.5
    elif num_channels == 6:
        init_offset = 4
...
def surpan(position, distance=1, num_channels=default_num_channels):
...
@player_method
def span(self, position, distance=1, num_channels=default_num_channels):
...
def srot(duration=16, num_channels=default_num_channels):
...
def mrot(duration=16, num_channels=default_num_channels):
    if isinstance(duration, int):
