...
class workspace:
    ...
    def __init__(self, CodeClass):
        ...
        self.version = this_version = '0.9.13.dev10'
        pypi_version = get_pypi_version()
        self.theme = COLOR_THEME

        def check_versions():
            # if pypi_version is not None and VersionNumber(pypi_version)
            # > VersionNumber(this_version):
            # tkMessageBox.showinfo("New version available", "There is a
            # new version of Renardo available from PyPI. Upgrade by going
    ...
    def set_window_title(self, text="Live Coding with Python & SuperCollider"):
    ...
    def toggle_transparency(self, event=None):
        """
        Sets the text and console background to black and then removes all
        black pixels from the GUI
        """
    ...
    def look(self, direction=-1):
        ...
        row, col = index(end)
        # If the col is 0, set the index to the end of the previous row
        # (unless first row)
        if direction == -1:
            if row == 1 and col == 0:
                return "1.0", "1.0"
    ...
    def duplicate_line(self, event=None):
        """
        Called using Ctrl+D - duplicates the content of this line and moves
        the insert to the same place on the line below
        """
