...
class TextPrompt:
    ...
    def __init__(self, root):
        ...
        other = ["SynthDefs"]
        self.namespace = sorted(list(set(keywords + synthdefs + attributes +
                                         player_methods + pattern_methods +
                                         scales + other)))

        self.values = [StringVar() for n in range(self.num_items)]
        self.clear()

        self.labels = [Label(self.master,
    ...
    def get_start_of_word(self):
        """
        Returns the TK index and characters directly preceeding the
        INSERT marker
        """
    ...
    def show(self):
        ...
        if bbox is not None:
            ...
            # If there is only 1 suggestion and we're at the end of the word
            # (or no suggestions), just hide
