...
from renardo.settings_manager import settings, DEFAULT_SAMPLES_PACK_NAME, SAMPLES_DOWNLOAD_SERVER
...
def is_default_spack_initialized():
    return (Path(settings.get("SAMPLES_DIR")) / DEFAULT_SAMPLES_PACK_NAME / 'downloaded_at.txt').exists()
...
def download_default_sample_pack(logger=None):
    ...
    download_files_from_json_index_concurrent(
        ...
        download_dir=settings.get("SAMPLES_DIR"),
        logger=logger
    )

    try:
        with open(Path(settings.get("SAMPLES_DIR")) / DEFAULT_SAMPLES_PACK_NAME / 'downloaded_at.txt', mode="w") as file:
