...
// FX parameter set (normalized 0.0-1.0)
pub static mut TRACK_FX_SET_PARAM_NORMALIZED: Option<extern "C" fn(*mut c_void, c_int, c_int, f64) -> bool> = None;

// Tempo
pub static mut MASTER_GET_TEMPO: Option<extern "C" fn() -> f64> = None;
pub static mut SET_CURRENT_BPM: Option<extern "C" fn(*mut c_void, f64, bool)> = None;
...
pub fn initialize_api(plugin_info: &ReaperPluginInfo) {
    unsafe {
        if let Some(get_func) = plugin_info.get_func {
            ...
            // FX set + tempo
            init_function_pointer(&get_func, "TrackFX_SetParamNormalized", &mut TRACK_FX_SET_PARAM_NORMALIZED);
            init_function_pointer(&get_func, "Master_GetTempo", &mut MASTER_GET_TEMPO);
            init_function_pointer(&get_func, "SetCurrentBPM", &mut SET_CURRENT_BPM);
        }
    }
}
