...
class AbletonProject:
        ...
        try:
            for clip_idx, clip in enumerate(track.clips):
                if clip is not None:
                    ...
                    self._clip_map[track_name]["by_original_name"][clip.name] = (
                        clip_info
                    )
        ...
        for track_idx, track in enumerate(self._set.tracks):
            ...
            try:
                ...
                if volume_param is not None:
                    ...
                    # Store track volume in parameter map
                    self._parameter_map[param_key] = {
                        "track_idx": track_idx,
                        "device_idx": -1,  # -1 indicates track-level parameter
                        "param_idx": -1,
                ...
                if pan_param is not None:
                    ...
                    # Store track pan in parameter map
                    self._parameter_map[param_key] = {
                        "track_idx": track_idx,
                        "device_idx": -1,  # -1 indicates track-level parameter
                        "param_idx": -1,
                ...
                # Silently ignore if mixer parameters can't be accessed
                pass
            ...
            for device_idx, device in enumerate(track.devices):
                ...
                for param_idx, parameter in enumerate(device.parameters):
                    ...
                    self._track_map[track_name]["devices"][device_name]["parameters"][
                        param_name
                    ] = {
                        "index": param_idx,
                        "parameter": parameter,
    ...
    def get_parameter_info(
        self, param_fullname: str, track_name: Optional[str] = None
    ) -> Optional[Dict]:
        """
        ...
        if track_name:
            ...
            if track_info:
                ...
                for full_param_name in self._parameter_map:
                    if full_param_name.startswith(
                        track_snake + "_"
                    ) and full_param_name.endswith("_" + param_fullname):
                ...
                if "_" not in param_fullname and len(track_info["devices"]) > 0:
                    ...
                    first_device_param = (
                        f"{track_snake}_{first_device_name}_{param_fullname}"
                    )
    ...
    def freeze_timevars_for_track(self, track_name: str) -> None:
        ...
        with self._timevar_lock:
            ...
            for param_name, (timevar, parameter) in list(self._timevar_params.items()):
                ...
                if (
                    param_name.startswith(track_name + "_")
                    or self._parameter_map.get(param_name, {}).get("track_snake")
                    == track_name
                ):
            ...
            for param_name, timevar, parameter in params_to_freeze:
                ...
                frozen_value = parameter.min + (
                    current_value * (parameter.max - parameter.min)
                )
    ...
    def set_parameter(
        self, param_fullname: str, value, track_name: Optional[str] = None
    ) -> bool:
        """
    ...
    def trigger_clip(self, track_name: str, clip_identifier: Any) -> bool:
        """
    ...
    def stop_all_clips(self) -> int:
        ...
        # Iterate through all tracks and stop all their clips using track's stop_all_clips method
        for track_name, track_info in self._track_map.items():
            track = track_info["track"]
            try:
                # Use the track's stop_all_clips method if available
                if hasattr(track, "stop_all_clips"):
                    track.stop_all_clips()
                    tracks_processed += 1
                else:
                    # Fallback: iterate through clips and stop each one
    ...
    def rescan_clips(self, print_clips: bool = True):
        ...
        if print_clips:
            ...
            try:
                ...
                if hasattr(runtime, "print_ableton_clips"):
                    runtime.print_ableton_clips()
            except:
                pass
    ...
    def change_ableton_bpm(self, bpm):
        ...
        Examples:
            ...
            from renardo.lib import linvar
        ...
        while self._timevar_running:
            ...
            for param_name, (timevar, parameter) in params_to_update:
                try:
                    ...
                    # Clamp to 0-1 and normalize to parameter range
                    current_value = max(0.0, min(1.0, current_value))
                    normalized_value = parameter.min + (
                        current_value * (parameter.max - parameter.min)
                    )
        ...
        for track_name, track_info in self._track_map.items():
            ...
            if clip_inventory and clip_inventory["by_index"]:
                ...
                for slot_idx, clip_info in sorted(clip_inventory["by_index"].items()):
                    print(
                        f"    [{slot_idx}] {clip_info['name']} (snake: {clip_info['name_snake']})"
                    )
            ...
            for device_name, device_info in track_info["devices"].items():
                ...
                for param_name, param_info in device_info["parameters"].items():
                    print(
                        f"    {param_name}: min={param_info['min']}, max={param_info['max']}, value={param_info['value']}"
                    )
