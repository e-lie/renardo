...
class RenardoApp:
    ...
    def __init__(self):
        ...
        if RenardoApp._instance is not None:
            raise RuntimeError(
                "RenardoApp is a singleton class. Use RenardoApp.get_instance() instead."
            )
    ...
    def launch(self):
        ...
        if self.args.launch_reaper:
            ...
            if reinit_reaper_with_backup():
                print(
                    "REAPER configuration has been backed up and will be reset on next launch"
                )
            else:
                print("Failed to backup and reinitialize REAPER configuration")
            return

        # Reaper integration (reapy deprecated - use reaper_backend instead)
        elif self.args.reapy:
            print(
                "Warning: --reapy option is deprecated. Please use --reaper-backend instead."
            )
            try:
                from renardo.reaper_backend.reaper_mgt import (
                    configure_reaper_integration,
                )
                ...
                print(
                    "Error: reaper backend not found. Please install required dependencies for Reaper integration."
                )
    ...
    def create_webapp_instance(self):
        ...
        if self.webapp is None:
            ...
            from renardo.sc_backend.supercollider_mgt import (
    ...
    def parse_args():
        ...
        parser.add_argument(
            ...
            "--pipe",
            action="store_true",
            help="run Renardo from the command line interface",
        )
        parser.add_argument(
            "-f",
            "--foxdot-editor",
            action="store_true",
            help="run Renardo with the classic FoxDot code editor",
        )
        parser.add_argument(
            "-c",
            "--create-scfiles",
            action="store_true",
            help="Create Renardo class file and startup file in SuperCollider user conf dir.",
        )
        parser.add_argument(
            "-N", "--no-tui", action="store_true", help="Don't start Renardo TUI"
        )
        parser.add_argument(
            "-g",
            "--use-gunicorn",
            action="store_true",
            help="Use Gunicorn to serve the web application (1 process, 10 threads)",
        )
        parser.add_argument(
            "--no-browser",
            action="store_true",
            help="Don't automatically open the web browser when starting the server",
        )

        # REAPER integration arguments
        reaper_group = parser.add_argument_group("REAPER Integration")
        reaper_group.add_argument(
            "-r",
            "--reapy",
            action="store_true",
            help="Enable REAPER integration with Reapy",
        )
        reaper_group.add_argument(
            "--launch-reaper",
            action="store_true",
            help="Launch REAPER with correct PYTHONHOME environment",
        )
        reaper_group.add_argument(
            "--initialize-reapy",
            action="store_true",
            help="Run interactive Reapy initialization process",
        )
        reaper_group.add_argument(
            "--reinit-reaper",
            action="store_true",
            help="Backup current REAPER config and reset to default",
        )
