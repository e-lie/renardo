...
from renardo.lib.Patterns.Main import Pattern, as_pattern, loop_pattern_func
from renardo.lib.Patterns.PGroups import (
    PGroupAnd, GeneratorPattern, PGroup, PGroupStar,
    PGroupPow, PGroupXor, PGroupOr,
    PGroupPlus, PGroupDiv, modulo_index
)
from renardo.lib.Patterns.Operations import LCM
from renardo.lib.Patterns.Generators import PRand
from renardo.lib.Utils import sliceToRange, EuclidsAlgorithm, PulsesToDurations
...
def PAlt(pat1, pat2, *patN):
    ...
    data = []
    item = [as_pattern(p) for p in [pat1, pat2] + list(patN)]
    size = LCM(*[len(i) for i in item])
    for n in range(size):
        for i in item:
            data.append(modulo_index(i, n))
    return Pattern(data)
...
def PZip2(pat1, pat2, rule=lambda a, b: True):
    ...
    i = 0
    while i < length:
        a, b = modulo_index(pat1, i), modulo_index(pat2, i)
        if rule(a, b):
            data.append((a,b))
        i += 1
    return Pattern(data)
...
def PRhythm(durations):
    ...
    *work in progress*
    ...
    if len(durations) == 1:
        ...
        durations = as_pattern(durations).data
        output = Pattern()
        for i in range(len(as_pattern(durations).data)):
            item = durations[i]
