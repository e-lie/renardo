...
def get_platform_info():
    ...
    if system == "linux":
        ...
        archive_name = (
            f"cpython-{PYTHON_VERSION}+20241016-{arch}-{variant}-install_only.{ext}"
        )
        ...
        archive_name = (
            f"cpython-{PYTHON_VERSION}+20241016-{arch}-apple-darwin-install_only.tar.gz"
        )
        ext = "tar.gz"
    ...
    if system == "linux":
        platform_suffix = f"{arch}-{variant}-install_only.{ext}"
    elif system == "darwin":
        platform_suffix = f"{arch}-apple-darwin-install_only.{ext}"
    else:  # windows
        platform_suffix = f"{arch}-pc-windows-msvc-shared-install_only.{ext}"

    return {
        "url": f"{BASE_URL}-{platform_suffix}",
...
def main():
    ...
    webclient_fresh_dir = script_dir / "webclient_fresh"
    python_dir = webclient_fresh_dir / "python"

    # Check if already installed
    if python_dir.exists():
        response = input(
            f"Python standalone already exists at {python_dir}. Replace it? (y/n): "
        )
        if response.lower() != "y":
    ...
    temp_dir = webclient_fresh_dir / "temp_python_download"
    ...
    try:
        ...
        print(
            f"✅ Python {PYTHON_VERSION} standalone successfully installed to {python_dir}"
        )
        ...
        if platform_info["system"] == "windows":
            ...
            python_exe = (
                python_dir / "bin" / f"python{'.'.join(PYTHON_VERSION.split('.')[:2])}"
            )
            print(f"\nPython executable: {python_exe}")
